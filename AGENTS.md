...
bactopia-py is a Python CLI companion package for [Bactopia](https://bactopia.io/), a flexible Nextflow pipeline for complete analysis of bacterial genomes. It provides 30 command-line tools: 22 user-facing commands for input preparation, public data search, environment building, dataset management, result summarization, pipeline linting, testing, and database integration; plus 8 pipeline utility scripts called from Nextflow module shell blocks.
...
The conda environment provides all dependencies: system tools (blast, pigz, wget), Python library deps, and dev/test tools. Poetry installs directly into the conda env (virtualenvs disabled via `poetry.toml`). Use `bactopia-dev` as the environment name.

```bash
conda env create -f environment.yml -n bactopia-dev
conda run -n bactopia-dev just install
```

### Running Commands

Use `conda run -n bactopia-dev` to run commands in the conda environment:

```bash
conda run -n bactopia-dev bactopia-download --version
conda run -n bactopia-dev bactopia-prepare --help
conda run -n bactopia-dev just fmt
conda run -n bactopia-dev just lint
```

### Common Tasks

```bash
conda run -n bactopia-dev just install       # poetry install --no-interaction
conda run -n bactopia-dev just fmt           # ruff format + ruff check --fix
conda run -n bactopia-dev just check-fmt     # check formatting without changes
conda run -n bactopia-dev just lint          # ruff check
conda run -n bactopia-dev just check         # check-fmt + lint
conda run -n bactopia-dev just build         # poetry build
conda run -n bactopia-dev just test          # pytest (accepts extra args)
conda run -n bactopia-dev just test-cov      # pytest with coverage report
conda run -n bactopia-dev just test-unit     # pytest -m "not integration"
```
...
bactopia/
  cli/                  # CLI commands (one module per command)
    ...
    docs.py             # Validate reference documentation
    ...
    scaffold.py         # Scaffold components from conda packages
    ...
    sysinfo.py          # Print system information
  ...
  lint/                 # Pipeline linting system
    ...
    rules/              # Lint rule definitions
      ...
      repo_rules.py         # Repo-level rules
      subworkflow_rules.py  # Subworkflow rules (S)
      workflow_rules.py     # Workflow rules (W)
  ...
  conda.py              # Conda environment utilities
  ...
  scaffold.py           # Scaffolding logic for new components
...
All 30 commands are defined in `pyproject.toml` under `[tool.poetry.scripts]`: 22 user-facing commands and 8 pipeline utility scripts. Every user-facing command uses `rich-click` and provides consistent flags: `--verbose`, `--silent`, `--version/-V`, `--help`.
