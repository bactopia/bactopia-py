...
def get_bactopia_files(path: str, name: str) -> dict:
    ...
    if is_bactopia:
        if not errors:
            bactopia_files["genome_size"] = parse_genome_size(
            ...
            for result_type in RESULT_TYPES:
                ...
                if result_type not in ["error", "generic", "kmers"]:
                    bactopia_files["files"][result_key] = getattr(
                        parsers, result_type
                    ).get_parsable_list(path, name)
    else:
        bactopia_files["ignored"] = True
        if name not in IGNORE_LIST:
            bactopia_files[
                "message"
            ] = f"'{path}/{name}' is does not look like Bactopia directory."
        else:
            bactopia_files[
                "message"
            ...
            for result in results:
                if result["missing"]:
                    if not result["optional"]:
                        ...
                        bactopia_results["missing"].append(
                            [result_type, result["files"]]
                        )
                    else:
                        bactopia_results["results"][result_type][
                            result["result_name"]
                        ] = {}
                else:
                    bactopia_results["results"][result_type][
